      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-4 mb-3 text-sm text-gray-600">
            <div className="flex items-center gap-1"><Mail className="w-4 h-4 text-blue-500" /><span>{applicant.email}</span></div>
            {applicant.phone && <div className="flex items-center gap-1"><Phone className="w-4 h-4 text-blue-500" /><span>{applicant.phone}</span></div>}
            {applicant.location && <div className="flex items-center gap-1"><MapPin className="w-4 h-4 text-blue-500" /><span>{applicant.location}</span></div>}
            {applicant.experience && <div className="flex items-center gap-1"><Briefcase className="w-4 h-4 text-blue-500" /><span>{applicant.experience} experience</span></div>}
          </div>
          {applicant.skills?.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-3">
              {applicant.skills.map(skill => (
                <span key={skill} className="bg-blue-100 text-blue-700 px-2.5 py-0.5 rounded-full text-xs font-medium">{skill}</span>
              ))}
            </div>
          )}
          ...
          {applicant.workExperience?.length > 0 && (
            <div className="flex items-start gap-2 text-sm text-gray-600">
              ...
              <span>{applicant.workExperience.map(w => `${w.jobTitle} at ${w.company}`).join(' · ')}</span>
            </div>
          )}
        </div>
        <button
      ...
      <div className="p-6 hover:bg-gray-50 transition-colors">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 text-gray-600 mb-2"><MapPin className="w-4 h-4" /><span>{job.location}</span></div>
          ...
          <div className="flex flex-col items-end gap-3 flex-shrink-0">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${isClosed ? 'bg-gray-100 text-gray-500' : 'bg-green-100 text-green-700'}`}>
      ...
      {expanded && (
        <div className="border-t border-gray-200 bg-gray-50 p-6">
          <h4 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
            ...
            Applicants
            {loaded && <span className="bg-blue-600 text-white text-xs px-2 py-0.5 rounded-full">{applicants.length}</span>}
          </h4>
          {loadingApplicants ? (
            <div className="flex items-center justify-center py-8 gap-3 text-gray-500">
              <Loader2 className="w-5 h-5 animate-spin" /><span>Loading applicants…</span>
            </div>
          ) : applicants.length === 0 ? (
            ...
            <div className="space-y-4">
              {applicants.map(applicant => (
                <ApplicantCard key={applicant.applicationId} applicant={applicant} jobTitle={job.title} />
              ))}
            </div>
          )}
        </div>
