...
export function Profile() {
  ...
  const handleEditToggle = () => {
    if (isEditing) { setEditedData(profileData); setEditedSkills(skills); setEditedWorkExp(workExperiences); }
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await saveProfile({
        fullName: editedData.name, jobTitle: editedData.title,
        phone: editedData.phone, location: editedData.location,
        yearsExperience: editedData.experience, aboutMe: editedData.about,
        skills: editedSkills,
        education: { degree: editedData.education, university: editedData.university, graduationYear: editedData.graduationYear },
        workExperience: editedWorkExp, certifications: uploadedCertificates,
      });
      if (updated?.user) localStorage.setItem('userData', JSON.stringify(updated.user));
  ...
  const handleAddWorkExp = () => {
    if (currentWorkExpJobTitle.trim() && currentWorkExpCompany.trim() && currentWorkExpDuration.trim()) {
      setEditedWorkExp([...editedWorkExp, { jobTitle: currentWorkExpJobTitle, company: currentWorkExpCompany, duration: currentWorkExpDuration }]);
      setCurrentWorkExpJobTitle(''); setCurrentWorkExpCompany(''); setCurrentWorkExpDuration('');
    }
  };

  const removeWorkExp = (i: number) => setEditedWorkExp(editedWorkExp.filter((_, idx) => idx !== i));
  const handleInputChange = (field: string, value: string) => setEditedData(prev => ({ ...prev, [field]: value }));
  ...
  const handleResumeUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const updated = await saveProfile({
          resumeBase64: base64, resumeFileName: file.name,
  ...
  const removeCertificate = (i: number) => setUploadedCertificates(prev => prev.filter((_, idx) => idx !== i));
    ...
    <div className="flex-1 flex flex-col overflow-y-auto">
      <main className="flex-1 p-8">
        <div className="max-w-4xl mx-auto">
                ...
                <div className="flex-1">
                  {isEditing ? (
                    <>
                      ...
                      <input type="text" value={editedData.title} onChange={(e) => handleInputChange('title', e.target.value)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[
                      { icon: <Mail className="w-4 h-4 text-blue-600" />, field: 'email', type: 'email' },
                      { icon: <Phone className="w-4 h-4 text-blue-600" />, field: 'phone', type: 'tel' },
                      { icon: <MapPin className="w-4 h-4 text-blue-600" />, field: 'location', type: 'text' },
                      { icon: <Briefcase className="w-4 h-4 text-blue-600" />, field: 'experience', type: 'text' },
                    ].map(({ icon, field, type }) => (
                      <div key={field} className="flex items-center gap-2 text-gray-600">
                        {icon}
                        {isEditing
                          ? <input type={type} value={(editedData as Record<string, string>)[field]} onChange={(e) => handleInputChange(field, e.target.value)} className={inp} />
                          : <span className="text-[15px]">{(profileData as Record<string, string>)[field]}</span>}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {/* Skills */}
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Award className="w-6 h-6 text-blue-600" /> Skills</h2>
            {isEditing ? (
              <div className="space-y-3">
                <div className="flex gap-2">
                  <input type="text" value={currentSkill} onChange={(e) => setCurrentSkill(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddSkill(); } }}
                    className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" placeholder="Type a skill and press Enter" />
              ...
              <div className="flex flex-wrap gap-2">
                {skills.length > 0 ? skills.map((skill) => (
                  <span key={skill} className="bg-gradient-to-r from-blue-100 to-purple-100 text-blue-700 px-4 py-2 rounded-full font-medium">{skill}</span>
          ...
          {/* Education */}
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2"><GraduationCap className="w-6 h-6 text-blue-600" /> Education</h2>
            {isEditing ? (
              <div className="space-y-4">
                {[['Degree', 'education', 'Bachelor of Science...'], ['University', 'university', 'Stanford University'], ['Graduation Year', 'graduationYear', '2020']].map(([label, field, placeholder]) => (
                  <div key={field}>
                    <label className="block text-gray-700 font-medium mb-2">{label}</label>
                    <input type="text" value={(editedData as Record<string, string>)[field]} onChange={(e) => handleInputChange(field, e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" placeholder={placeholder} />
                  </div>
                ))}
              </div>
            ) : (
          ...
          {/* Certificates */}
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Award className="w-6 h-6 text-purple-600" /> Certificates</h2>
              ...
              <div className="mb-4 space-y-2">
                {uploadedCertificates.map((cert, index) => (
                  <div key={index} className="p-3 bg-purple-50 border border-purple-200 rounded-lg flex items-center justify-between">
                    ...
                    <button onClick={() => removeCertificate(index)} className="p-1 hover:bg-purple-200 rounded transition-colors"><X className="w-4 h-4 text-purple-600" /></button>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Work Experience */}
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Briefcase className="w-6 h-6 text-blue-600" /> Work Experience</h2>
            {isEditing ? (
              <div className="space-y-4">
                <div className="space-y-3">
                  <div><label className="block text-gray-700 font-medium mb-2">Job Title</label>
                    <input type="text" value={currentWorkExpJobTitle} onChange={(e) => setCurrentWorkExpJobTitle(e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" placeholder="Senior Software Engineer" /></div>
                  <div><label className="block text-gray-700 font-medium mb-2">Company</label>
                    <input type="text" value={currentWorkExpCompany} onChange={(e) => setCurrentWorkExpCompany(e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" placeholder="Tech Solutions Inc." /></div>
                  <div><label className="block text-gray-700 font-medium mb-2">Duration</label>
                    <input type="text" value={currentWorkExpDuration} onChange={(e) => setCurrentWorkExpDuration(e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" placeholder="2020 - Present" /></div>
                ...
                {editedWorkExp.map((exp, index) => (
                  <div key={index} className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-start justify-between">
                      <div><h4 className="font-bold text-gray-900">{exp.jobTitle}</h4><p className="text-gray-600">{exp.company}</p><p className="text-sm text-gray-500">{exp.duration}</p></div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2"><FileText className="w-6 h-6 text-blue-600" /> Resume</h2>
