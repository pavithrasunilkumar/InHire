...
import { Briefcase, User, Home, LayoutDashboard, LogOut } from 'lucide-react';
          ...
          <div className="flex items-center gap-6">
            <nav className="flex items-center gap-2">
              {[
                ...
                { to: '/home/dashboard', icon: <LayoutDashboard className="w-5 h-5" />, label: 'Dashboard' },
                { to: '/home/profile', icon: <User className="w-5 h-5" />, label: 'Profile' },
              ].map(({ to, icon, label }) => (
                <Link key={to} to={to}
